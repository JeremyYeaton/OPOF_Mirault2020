...
home
disp(' ')
disp('PRE-PROCESSING DES DONNEES OPOF: Analysis set');
...
%     if (S_tmp == 29)% SUBJECT 29
%         EEG = pop_interp(EEG, 63, 'spherical'); %PO4
%         EEG = eeg_checkset( EEG );
%     end
    
%     if (S_tmp == 32)% SUBJECT 32
%         EEG = pop_interp(EEG, 63, 'spherical'); %PO4
%         EEG = eeg_checkset( EEG );
%     end
    
%     if (S_tmp == 37)% SUBJECT 37
%         EEG = pop_interp(EEG, 15, 'spherical'); %T7
%         EEG = eeg_checkset( EEG );
%     end
    disp(' ')
    ...
    path_to_save = 'C:\Users\LPC\Documents\OPOF_reanalyze\Results\C_analysisSet_preprocess';
